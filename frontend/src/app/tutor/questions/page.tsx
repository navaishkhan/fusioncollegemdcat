      ...
      >
        ...
        >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-2 flex flex-wrap gap-2">
                    <span className="rounded-full bg-purple-500/20 px-2 py-0.5 text-[10px] font-bold uppercase text-purple-300">{formatSubjectName(q.subject)}</span>
                  ...
                  {!selectMode && <p className="mt-2 text-[10px] text-cyan-400 font-bold opacity-60">Tap to view answer</p>}
                </div>
                {!selectMode && (
                  <div className="flex shrink-0 flex-col gap-1.5 z-10" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => router.push(`/tutor/questions/edit?id=${q.id}`)} className="rounded-lg bg-white/5 border border-white/10 px-3 py-1.5 text-[11px] font-bold text-white cursor-pointer hover:bg-white/10 transition-all">Edit</button>
                    <button onClick={() => setDeleteConfirmId(q.id)} className="rounded-lg bg-red-600/20 border border-red-500/30 px-3 py-1.5 text-[11px] font-bold text-red-300 cursor-pointer hover:bg-red-600/30 transition-all">Del</button>
                  </div>
                )}
              </div>
            </Card>
          ...
          <div
            ...
            className="w-full h-full"
          >
            <Card className="h-full border-cyan-500/30 bg-white/5 backdrop-blur-xl glossy-border">
    ...
    <AuthGuard roles={["admin", "tutor"]}>
      <PageShell title="Question Bank">
        ...
        <div className="mb-4 flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          ...
          {SUBJECTS.map((s) => (
            <button key={s} onClick={() => setSubject(s)} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold cursor-pointer ${subject === s ? "bg-cyan-500/20 text-cyan-400" : "bg-white/5 text-zinc-400"}`}>{formatSubjectName(s)}</button>
          ))}
        </div>
        ...
        <div className="mb-4 relative flex items-center">
          <input
            ...
            className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 pl-10 text-sm text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-cyan-500/50 focus:bg-white/8 transition-all"
          />
          <svg
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-3.5 text-zinc-500 hover:text-white transition-colors text-xs cursor-pointer"
            >
              x
            </button>
          )}
        ...
        {/* Category Group Tabs */}
        {tabs.length > 1 && (
          <div className="mb-4 rounded-xl bg-white/5 border border-white/10 p-1 flex gap-1">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex-1 rounded-lg py-2 text-center text-[11px] font-semibold transition-all cursor-pointer ${
                  activeTab === tab.id
        ...
        {subTopics.length > 1 && (activeTab === "chapter_wise" || activeTab === "past_papers") && (
          <div className="mb-4 flex gap-1.5 overflow-x-auto pb-2 scrollbar-none">
            <button
              ...
              className={`shrink-0 rounded-lg px-2.5 py-1.5 text-[10px] font-bold border transition-all cursor-pointer ${
                !selectedSubTopic
            ...
            {subTopics.map((topic) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 rounded-lg px-2.5 py-1.5 text-[10px] font-bold border transition-all capitalize cursor-pointer ${
        ...
        <div className="mb-4 flex items-center gap-2">
          {!selectMode ? (
            <>
              <button onClick={() => router.push("/tutor/questions/new")} className="flex-1 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-700 py-3 text-sm font-bold text-white hover:bg-emerald-500 transition-colors magnetic-btn cursor-pointer shadow-lg shadow-emerald-950/20">+ New</button>
              <button onClick={() => router.push("/tutor/questions/import")} className="rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 transition-colors cursor-pointer">Import</button>
              {filteredQuestions.length > 0 && (
                <button
                  onClick={() => setSelectMode(true)}
                  className="flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 py-3 text-xs font-semibold text-zinc-400 hover:text-white transition-colors cursor-pointer"
                >
          ...
          <div className="space-y-3" style={{ perspective: 1000 }}>
            {sourceFilter === "tutor" ? (
              <div className="space-y-8">
                {SUBJECTS.map((sub) => {
                  const subQuestions = filteredQuestions.filter(q => q.subject === sub);
                  ...
                  return (
                    <div key={sub} className="space-y-4">
                      <div className="flex items-center gap-2 border-b border-white/5 pb-2 mb-3">
                        ...
                        <span className="rounded-full bg-white/5 border border-white/10 px-2 py-0.5 text-[10px] font-bold text-zinc-400">
                          {subQuestions.length} {subQuestions.length === 1 ? "MCQ" : "MCQs"}
                        </span>
                      </div>
                      <motion.div
                        initial="hidden"
                      ...
                      >
                        <AnimatePresence>
                          {subQuestions.map(q => renderQuestionCard(q))}
                        </AnimatePresence>
                      </motion.div>
                    </div>
                  );
                ...
                {filteredQuestions.length === 0 && (
                  <p className="text-center text-sm text-zinc-500 py-8">No questions found.</p>
                )}
              </div>
            ) : (
              <motion.div
            ...
            {sourceFilter !== "tutor" && filteredQuestions.length === 0 && (
              <p className="text-center text-sm text-zinc-500 py-8">No questions found.</p>
            )}
          </div>
        )}
