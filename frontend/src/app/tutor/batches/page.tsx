    ...
    <AuthGuard roles={["admin", "tutor"]}>
      <PageShell title="Batches">
        ...
        <button
          ...
          className="mb-4 w-full rounded-xl bg-gradient-to-r from-emerald-600 to-teal-700 py-3 text-sm font-bold text-white magnetic-btn cursor-pointer shadow-lg shadow-emerald-950/20"
        >
          {showCreate ? "Cancel" : "+ New Batch"}
        </button>

        {showCreate && (
          <div className="mb-4 space-y-3 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-4 glossy-border">
            <input
              value={batchName}
            ...
            <button
              ...
              className="w-full rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 py-2.5 text-sm font-bold text-white disabled:opacity-50 magnetic-btn cursor-pointer shadow-lg shadow-cyan-950/20"
            >
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    ...
                    className="shrink-0 rounded-lg bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border border-cyan-500/20 px-3 py-1 text-xs font-semibold text-cyan-400 hover:text-white transition-all cursor-pointer"
                  >
                    Enroll
                  </button>
                </div>
                  ...
                  <div className="mt-3 space-y-2 border-t border-white/5 pt-3">
                    <div className="flex gap-2">
                      <select
                        ...
                        className="flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500/40 focus:bg-white/8 transition-all"
                      >
                      ...
                      <button
                        ...
                        disabled={enrolling}
                        className="rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-50 magnetic-btn cursor-pointer"
                      >
                        {enrolling ? "..." : "Add"}
                      </button>
                    </div>
                    {enrolledStudents.length > 0 && (
                      <div className="space-y-1">
                        ...
                        {enrolledStudents.map((s) => (
                          <div
                            key={s.id}
                            className="flex items-center justify-between rounded-lg bg-white/3 border border-white/5 px-3 py-1.5"
                          >
